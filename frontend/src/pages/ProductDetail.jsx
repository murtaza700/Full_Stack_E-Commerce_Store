...
const ProductDetails = () => {
    ...
    if (productLoading?.fetchOne) {
        return (
            <>
                <Meta
                    title="Loading Fragrance Profile"
                    description="Synchronizing bespoke luxury formulation blueprints and olfactory note characteristics registries with database cluster indices."
                />
    ...
    if (!product) {
        return (
            <>
                <Meta
                    title="Fragrance Profile Not Found"
                    description="The targeted boutique perfume formulation document reference index could not be located inside our system inventory vaults bounds."
                />
    ...
    return (
        <>
            <Meta
                title={`${product.title}`}
                description={`${product.description?.substring(0, 160) || 'Discover this unique, premium artisan perfume formulation from the exclusive Scentsô boutique.'}`}
                keywords={`${product.title?.toLowerCase()}, luxury perfume, scentsô signature oud, premium cologne, artisan fragrance, ${product.category?.name?.toLowerCase() || 'fragrances'}`}
            />
